...
//PushPullConcurrentQueue是针对Spsc类型的设计，用于提高性能
//这里需要注意一点因为存储offerIndex以及pollIndex采用的是LongAdder，所以在每次计算时都需要调用sum来进行计算
//为了避免浪费性能 这里采用了headCache以及tailCache的方式来缓存head以及tail的值，这里主要是针对生产和消费的速度不一致情况下的优化
public class PushPullConcurrentQueue<E> extends PushPullConcurrentQueuePadding4 implements ConcurrentQueue<E> {
    ...
    public boolean offer(final E e) {
        if (e != null) {
            ...
            //判断该位置是否被消费掉了，举个例子如下：
            //假设size = 2，tail = 0，那么queueStart = -2，而head 此时 = 0
            // 所以 head.sum() > queueStart 成立表示该位置的数据已经被消费掉了或者还未被生产
    ...
    public E poll() {
        ...
        //计算对应位置是否有生产的数据，如果没有生产的数据则不需要进行消费
        //head < tail.sum() 表示该位置还有数据未被消费，举个例子如下：
        //假设ring buffer 大小为2，[null, 1]
        //当我们消费为1这个元素时，head = 1 ，head < tail.sum() 表达成立，说明该元素还未被消费，可以正常消费
        //当我们消费完成1这个元素后再次进行消费时这个时候tail = 2，head = 2 ，head < tail.sum() 表达式不成立
        if ((head < tailCache) || (head < (tailCache = tail.sum()))) {
            //计算消费index的位置
            final int dx = (int) (head & mask);
            //获取对应的元素
            final E e = buffer[dx];
            //将对应index位置的元素设置为null
            buffer[dx] = null;
            //将pollIndex + 1
            this.head.increment();
            return e;
